if Code.ensure_loaded?(Mint.HTTP) do
  defmodule Graphd.Adapters.HTTP do
    use Graphd.Adapter
    ...
    defp parse_success(:alter, %{"data" => data}) do
      Graphd.Api.Payload.new(Data: data)
    end

    defp parse_success(:mutate, %{"data" => %{"uids" => uids, "queries" => queries}} = response) do
      Graphd.Api.Response.new(txn: parse_txn(response), uids: uids, json: queries)
    end

    defp parse_success(:query, %{"data" => data} = response) do
      Graphd.Api.Response.new(txn: parse_txn(response), json: data)
    end
    ...
    defp parse_txn(_, aborted) do
      Graphd.Api.TxnContext.new(aborted: aborted)
    end
